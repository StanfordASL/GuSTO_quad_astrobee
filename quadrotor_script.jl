# -----------------------------------------------------------------------------------------------
# -   Initializing script for the quadrotor example - Thomas Lew and Riccardo Bonalli 12/2019   -
# -----------------------------------------------------------------------------------------------
...
function plt_solutions(scp_problem::GuSTOProblem, model, X_all, U_all;
    ...
    plt.ylim(ylims)
    if B_plot_labels
        plt.title("Open-Loop Quadcopter Trajectories")
        plt.xlabel("E")
        plt.ylabel("N")    
...
function plt_final_solution(scp_problem::GuSTOProblem, model, X, U)
    ...
    # rcParams["font.size"] = 15
    # rcParams["font.family"] = "Helvetica"
    plt.title("Final Quadcopter Trajectory")
    plt.xlim([-0.5,3.])
    plt.ylim([ 0.0,6.])
    plt.xlabel("E")
    plt.ylabel("N")    
